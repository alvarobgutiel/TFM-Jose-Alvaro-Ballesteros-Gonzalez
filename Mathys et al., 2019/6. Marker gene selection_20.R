...
jpeg(file="./figures/6.Marker Gene Selection_20/Heatmap_clusters.jpeg", width=6, height=17, units="in", res=300)
...
#Colect the top 25 genes for each cell type and put them into a single table
top25_cell_types = lapply( names(markers_genes_cell_types), function(x) { temp = markers_genes_cell_types[[x]][1:25, 1:2] ; temp$gene = rownames(markers_genes_cell_types[[x]])[1:25] ; temp$cell.type = x ; return(temp) } )
top25_cell_types = as_tibble(do.call(rbind, top25_cell_types))
write.csv(top25_cell_types, file = "top25_cell_types_20.csv")


#25 gene expression of each cell type
dir.create("./figures/6.Marker Gene Selection_20/cell_types")
for(i in unique(top25_cell_types$cell.type)){
  ...
  barplot( sort( setNames(-log10(top25_cell_types$p.value), top25_cell_types$gene) [top25_cell_types$cell.type == i], F),
           horiz = T,las=1 ,main=paste0(i," vs. rest"),border = "white", yaxs="i",xlab="-log10FC" )
  abline(v=c(0,-log10(0.05)),lty=c(1,2))
  dev.off()
}
...
#HeatMap
as_tibble(top25_cell_types) %>% group_by(cell.type)  %>% top_n(-5, p.value) -> top5_cell_type
jpeg(file="./figures/6.Marker Gene Selection_20/Heatmap_cell_type_1.jpeg", width=6, height=8, units="in", res=300)
